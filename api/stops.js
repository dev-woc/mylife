...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    const rows = await sql`
      ...
      FROM day_stops WHERE date = ${date} ORDER BY position
    `;
  ...
  if (req.method === "POST") {
    ...
    const [{ max }] = await sql`SELECT COALESCE(MAX(position), 0) as max FROM day_stops WHERE date = ${date}`;
    const position = Number(max) + 1;
    const [row] = await sql`
      INSERT INTO day_stops (date, position, name, address, lat, lng)
      VALUES (${date}, ${position}, ${name}, ${address}, ${lat ?? null}, ${lng ?? null})
  ...
  if (req.method === "PATCH") {
    ...
    await sql`UPDATE day_stops SET lat = ${lat}, lng = ${lng}, updated_at = NOW() WHERE id = ${id}`;
  ...
  if (req.method === "DELETE") {
    ...
    const [deleted] = await sql`DELETE FROM day_stops WHERE id = ${id} RETURNING position`;
    if (deleted) {
      await sql`
        UPDATE day_stops SET position = position - 1, updated_at = NOW()
        WHERE date = ${date} AND position > ${deleted.position}
      `;
    }
    return res.status(200).json({ ok: true });
  }
